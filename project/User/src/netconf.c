...
void con_to_server(void)
{
	...
	if( (order_netconn = netconn_new(NETCONN_TCP)) != NULL)
	{
		NET_DEBUG_PRINT(ORDER_CONNECTION_BUILD);
	}
	else
	{
		NET_DEBUG_PRINT(ERROR_ORDER_CONNECTION_FAIL_TO_BUILD);
...
void LwIP_Init(void)
{
  ...
  localhost_gw.addr = 0;
	NET_DEBUG_PRINT(DHCP_CAN_BE_CHOOSED);
#else
//以下为手动分配IP
...
//JockJo家中测试
  ...
  netif_set_default(&DM9161_netif);
		...
		{
				NET_DEBUG_PRINT(ERROR_DHCP_START_FAILED);
				//此处应该有不成功的处理函数为佳
		}
		while(DM9161_netif.ip_addr.addr == 0)
		{
			//等待一会，让IP地址存放近DM9161
				OSTimeDlyHMSM(0, 0, 1, 0);
				NET_DEBUG_PRINT(ERROR_DM9161_NETIF_IS_NULL);
		}
	...
	{
		NET_DEBUG_PRINT(STR_IPADDR_IS);		
		NET_DEBUG_PRINT(str_ipaddr);
	}
	else
	{
		NET_DEBUG_PRINT(ERROR_STR_IPADDR_IS_NULL);		
	}
